...
class Match:
    ...
    async def ask_to_cancel(self, trigger_message):
        ...
        if self.cancellation_aborted == False:
            message_text = (
                "```Cancelling match in 5 seconds. Press on ❌ react to stop cancellation.\n"
    ...
    async def start(self):
        ...
        await self.send_message(
            text=intro_text,
            file=discord.File(self.shrouded_path),
            section="shrouded_image",
        )
    ...
    async def end(self, nature, winner=str()):
        ...
        if nature == "failure":
            await self.send_message(
                f"Time's up! It's {self.pokemon_name}!",
                section="end",
                file=discord.File(self.unshrouded_path),
            )

        elif nature == "success":
            await self.send_message(
                f"That's right, {winner.mention}! It's {self.pokemon_name}!",
                section="end",
                file=discord.File(self.unshrouded_path),
            )
...
async def poke(ctx, *, generation_string="all"):
    ...
    if await can_start_match(
        server_id=server_id, channel_id=channel_id, user_id=ctx.message.author.id
    ):

        matches[ctx.message.channel.id] = Match(
            ctx, generation_string=generation_string
        )
    ...
    if reaction.message.channel.id in matches.keys() and user.id != bot.user.id:
        ...
        if "cancel_dialogue" in match.messages:
            if reaction.message.id == match.messages["cancel_dialogue"].id:
                ...
                # Bad
                await match.send_message(
                    "Cancellation aborted. ", section="cancel_dialogue"
                )
